        ...
        <tr v-for="item in srtlines" :key="item.id">
          <td width="50%">
            <textarea
              ...
              type="text"
              v-model="item.text"
              @blur="souceChange(item.id)"
              @focus="forcusText(item.id)"
            />
            <div class="baidutrans">{{ item.trans }}</div>
          </td>
          <td>
            <textarea
              type="text"
              class="usertrans"
              v-model="item.trans2"
              @blur="transChange(item.id)"
              @focus="forcusTrans(item.id)"
            />
          </td>
        </tr>
      </table>
...
export default {
  ...
  created() {
    ...
    if (this.$route.params.mobile) this.mobile = true;
